/* Plugin Template generated by Pawn Studio */

#include <sourcemod>
#include <sdkhooks>
#include <CodD0_engine>
#include <CodD0/skills/CodD0_skill_archangelWings>
#include <CodD0/skills/CodD0_skill_gravity>

public Plugin myinfo =  {
	name = "CodD0 Perk: Archangel Wings",
	author = "d0naciak",
	description = "Archangel Wings",
	version = "1.0",
	url = "d0naciak.pl"
}

int g_perkID;
bool g_hasPerk[MAXPLAYERS + 1];
public void OnPluginStart() {
	g_perkID = CodD0_RegisterPerk("Skrzydła Archanioła", "Użyj w powietrzu, aby wywołać trzęsienie ziemii");
}

public void OnPluginEnd() {
	CodD0_UnregisterPerk(g_perkID);
}

public Action CodD0_PerkChanged(int client, int perkID, int perkValue) {
	if(g_perkID != perkID && g_hasPerk[client]) {
		CodD0_SetClientArchangelWings(client, CodD0_SkillSlot_Perk, 0, 0.0, 0.0);
		CodD0_SetClientGravity(client, CodD0_SkillSlot_Perk, 1.0);
		g_hasPerk[client] = false;
	}
	
	return Plugin_Continue;
}

public void CodD0_PerkChanged_Post(int client, int perkID, int perkValue) {
	if (perkID == g_perkID && !g_hasPerk[client]) {
		CodD0_SetClientArchangelWings(client, CodD0_SkillSlot_Perk, 800, 95.0, 0.8);
		CodD0_SetClientGravity(client, CodD0_SkillSlot_Perk, 0.5);
		g_hasPerk[client] = true;
	}
}

public void CodD0_PerkUsed(int client, int perkID) {
	if (g_hasPerk[client] && CodD0_UseArchangelWings(client, CodD0_SkillSlot_Perk) == CodD0_SkillPrep_Available) {
		CodD0_ClientPerkGotReady(client, 5.0);
	}
}