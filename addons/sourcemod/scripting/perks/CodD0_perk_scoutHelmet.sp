...
public Plugin myinfo =  {
	name = "CodD0 Perk: Scout's helmet",
	author = "d0naciak",
	description =  "Scout's helmet",
	version = "1.0",
	url = "d0naciak.pl"
}
...
public void OnPluginStart() {
	g_perkID = CodD0_RegisterPerk("Kask Zwiadowcy", "Nie otrzymujesz obrażeń w głowę");
}
...
public Action CodD0_PerkChanged(int client, int perkID, int perkValue) {
	if(g_perkID != perkID && g_hasPerk[client]) {
		CodD0_SetClientHitBoxRedDmgMultiplier(client, CodD0_SkillSlot_Perk, HIT_HEAD, 1.0);
...
public void CodD0_PerkChanged_Post(int client, int perkID, int perkValue) {
	if(g_perkID == perkID && !g_hasPerk[client]) {
		CodD0_SetClientHitBoxRedDmgMultiplier(client, CodD0_SkillSlot_Perk, HIT_HEAD, 0.0);
